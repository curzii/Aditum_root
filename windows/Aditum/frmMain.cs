    ...
    public partial class frmMain : Form
    {
        ...
        private void btnRunAditumServer_Click(object sender, EventArgs e)
        {
            ...
            if (HostIsLive)
            {
                SyncTime();
                rtbxOutput.AppendText(System.Environment.NewLine);
                rtbxOutput.AppendText(DateTime.Now.ToString("[HH:mm:ss] ") + "[ ] Starting server on Target...");
                string hostip = tbxHostIP.Text;
                string hostusername = tbxHostUsername.Text;
                string hostpassword = tbxHostPassword.Text;
        ...
        public void getlogs()
        {
            ...
            try
            {
                ...
                {
                    rtbxOutput.AppendText(System.Environment.NewLine);
                    rtbxOutput.AppendText(DateTime.Now.ToString("[HH:mm:ss] ") + "[-] Host is down.");
                }
                if (!cbxLogs.Checked || (cbxLogs.Checked && !HostIsLive ))
                {
                    if (openFileDialog1.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                    {
                        Stream file = File.Open(openFileDialog1.FileName, FileMode.Open);
                        using (Stream s = File.Create("log.csv"))
                        {
                            file.CopyTo(s);
                        }
                        file.Close();
                    ...
                    {
                        rtbxOutput.AppendText(System.Environment.NewLine);
                        rtbxOutput.AppendText(DateTime.Now.ToString("[HH:mm:ss] ") + "[-] No file selected.");
                        rtbxOutput.AppendText(System.Environment.NewLine);
                        rtbxOutput.AppendText(DateTime.Now.ToString("[HH:mm:ss] ") + "[-] No action taken.");
                        return;
                    }
                }
                try
                ...
                {
                    rtbxOutput.AppendText(System.Environment.NewLine);
                    rtbxOutput.AppendText(DateTime.Now.ToString("[HH:mm:ss] ") + "[-] Operation failed.");
                    rtbxOutput.AppendText(System.Environment.NewLine);
                    rtbxOutput.AppendText(DateTime.Now.ToString("[HH:mm:ss] ") + "[-] Could not process the CSV file.");
                    rtbxOutput.AppendText(System.Environment.NewLine);
                    rtbxOutput.AppendText(DateTime.Now.ToString("[HH:mm:ss] ") + "[-] Check CSV file format.");
                }
            }
            catch (Exception)
            {
            ...
            if (dt.Rows.Count != 0)
            {
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    try
                    {
                        ...
                        n.Add(dgv["DETAILS", i].Value.ToString());
                    }
                    catch (Exception)
                    {
                        ;
        ...
        private void clbxShowSelected_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                if (clbxShowSelected.GetItemChecked(clbxShowSelected.SelectedIndex) == false)
                {
                    clbxShowSelected.SetItemChecked(clbxShowSelected.SelectedIndex, true);
                }
                else
                {
                    clbxShowSelected.SetItemChecked(clbxShowSelected.SelectedIndex, false);
                }
            }
            clbxShowSelected_SelectedIndexChanged(null, null);
        }
    }
}
